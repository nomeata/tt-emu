...
class MtDebugger:
    ...
    def poll_transition(self) -> Transition | None:
        ...
        try:
            ...
            pass
        # Pre-render the line here (where MT's state_name is in scope) so the neutral
        # Transition.format() needs no per-firmware state table.
        old = state_name(previous[-1]) if previous else "(pre-init)"
        new = state_name(chain[-1]) if chain else "?"
        depth = f"depth {max(len(previous) - 1, 0)}→{len(chain) - 1}"
        oid_note = f"  (last tap: OID {last_oid})" if last_oid else ""
        text = f"{kind:7} {old} → {new}  [{depth}]{oid_note}"
        return Transition(
            kind=kind, old_chain=previous, new_chain=chain, last_oid=last_oid, text=text
        )
    ...
    def _snapshot(self) -> DebugSnapshot:
        ...
        if chain is None:
            return DebugSnapshot(ready=self.ready)
