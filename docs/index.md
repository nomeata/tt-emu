...
|------|-----------------------------------------------------|
| `memory-map-and-boot.md` | ARM926EJ-S core; full address map; artifacts in `update3202MT.upd`; the **boot recipe** — enter PC=`0x08039100`, SP=`0x08420000`, map PROG@`0x08009000` + nandboot@`0x08000000`/`0x07ff8000`; the RAM globals table; the from-entry seeds (geometry struct `0x08008ca8`, QHsm byte `0x08007e80`, `0x07ffe740`→1); boot self-tests & health checkpoints. |
...
| `0x08400000` | Stack/heap headroom (SVC stack top `0x08420000`, IRQ `0x0841f000`) | memory-map-and-boot |
...
| what | value | doc |
|---|---|---|
| CPU entry / stacks | PC=`0x08039100`, SP(SVC)=`0x08420000`, SP(IRQ)=`0x0841f000`, SVC mode, IRQs enabled | memory-map-and-boot §5.2 |
